program test_sweeper
  ...
  use state, only: initialize_state, phi, psi, source, phistar, internal_source
  use sweeper
  ...
  double precision :: phi_test(1,7), psi_test(1,4,7),source_test(1,4,7),phistar_test(1,0:7,7)
  ...
  call update_phistar()
  phistar_test(:,0,:) = 1.0
  t1 = testCond(norm2(phistar-phistar_test) .lt. 1e-7)
