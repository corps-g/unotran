module sweeper
  ...
  subroutine sweep(sig_s, sig_t, nu_sig_f, chi, phi, psi, source, incoming)
    ...
    do o = 1, 2  ! Sweep over octants
      ...
      do c = cmin, cmax, cstep  ! Sweep over cells
        do a = amin, amax, astep  ! Sweep over angle
          ...
          Q = updateSource(source(:,an,c), phi_old(:,:,c), c, an, sig_s(:,:,:,mMap(c)), nu_sig_f(:,mMap(c)), chi(:,mMap(c)))
  ...
  function updateSource(Sg, Phig, cell, angle, sig_s, nu_sig_f, chi)
