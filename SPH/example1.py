...
def plot(ref, homo):
    ...
    plt.figure(0)
    plt.plot(rxn.flatten(), label='reference')
    plt.plot([h for h in rxn_homo.flatten() for _ in range(ncells)], label='homogenized')
    plt.plot([h for h in rxn_sph.flatten() for _ in range(ncells)], label='sph')
    plt.legend()
    plt.xlabel('spatial cell')
    plt.ylabel('absorption rate')

    plt.figure(1)
    plt.plot(ref.phi.flatten(), label='reference')
    plt.plot([h for h in ref.phi_homo.flatten() for _ in range(ncells)], label='homogenized')
    plt.plot([h for h in homo.phi_homo.flatten() for _ in range(ncells)], label='sph')
    plt.legend()
    plt.xlabel('spatial cell')
...
def runSPH(G, pin_map, xs_name):
    ...
    for i in range(1000):
        ...
        mu = ref.phi_homo / homo.phi_homo  # / np.linalg.norm(ref.phi_homo, axis=1) * np.linalg.norm(homo.phi_homo, axis=1)
...
if __name__ == '__main__':
    ...
    uo2low = runSPH(1, [1, 1, 1, 1, 1, 1, 1, 1], xs_name)
    uo2high = runSPH(1, [0, 0, 0, 0, 0, 0, 0, 0], xs_name)
    ...
    ref_errs = -np.array([-0.07, -0.1, -0.16, -0.26, -0.43, -0.69, -1.00, -1.22, 1.57, 0.9, 0.45, 0.22, 0.1, 0.04, 0.02, 0.02])

    print 'Reference reaction rates'
    print rxn_ref
    print 'Homogenzied reaction rates'
    print rxn_homo
    print ref_errs / 100 * rxn_ref + rxn_ref
